    ...
    <el-table v-loading="loading" :data="i18nList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="语种" align="center" prop="lang">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_language" :value="scope.row.lang"/>
        </template>
      </el-table-column>
...
<script>
import { listI18n, getI18n, delI18n, addI18n, updateI18n } from "@/api/i18n/i18n";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除国际化编号为"' + ids + '"的数据项？').then(function() {
        return delI18n(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('i18n/i18n/export', {
        ...this.queryParams
